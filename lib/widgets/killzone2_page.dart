...
class Killzone2Page extends StatefulWidget {
  const Killzone2Page({super.key});

  @override
  State<Killzone2Page> createState() => _Killzone2PageState();
}

class _Killzone2PageState extends State<Killzone2Page> {
  double _rating = 0;
  ...
  Future<void> _loadPrefs() async {
    ...
    setState(() {
      _rating = prefs.getDouble('killzone2_rating') ?? 0;
      _favorite = prefs.getBool('killzone2_favorite') ?? false;
      _review = prefs.getString('killzone2_review') ?? '';
  ...
  Future<void> _loadReviews() async {
    final prefs = await SharedPreferences.getInstance();
    setState(() {
      _reviews = prefs.getStringList('killzone2_reviews') ?? [];
    });
  }

  Future<void> _savePrefs() async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setDouble('killzone2_rating', _rating);
    await prefs.setBool('killzone2_favorite', _favorite);
    await prefs.setString('killzone2_review', _review);
  }

  Future<void> _addReview(String review) async {
    if (review.trim().isEmpty) return;
    final prefs = await SharedPreferences.getInstance();
    final reviews = prefs.getStringList('killzone2_reviews') ?? [];
    reviews.add(review.trim());
    await prefs.setStringList('killzone2_reviews', reviews);
    setState(() {
      _reviews = reviews;
      _review = '';
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text('Killzone 2', style: TextStyle(color: textColor)),
          ...
          children: [
            Center(
              child: ClipRRect(
                ...
                child: Image.asset(
                  'assets/images/killzone2_card.jpg',
                  width: 300,
                  height: 160,
                  fit: BoxFit.cover,
                ),
            ...
            Text('Killzone 2', style: Theme.of(context).textTheme.headlineMedium?.copyWith(fontWeight: FontWeight.bold, color: textColor)),
            ...
            Wrap(
              ...
              children: [
                ...
                Chip(label: Text('PlayStation 3', style: TextStyle(color: textColor)), backgroundColor: cardColor),
              ],
            ),
...
''', style: TextStyle(color: textColor)),
            ...
            Text(
              'Killzone 2 is a gritty, atmospheric first-person shooter set on the planet Helghan. As Sev, a member of the ISA, you must battle the Helghast in a cinematic campaign with intense firefights and stunning visuals.',
              style: TextStyle(fontSize: 16, color: textColor),
            ),
            _buildSectionTitle('Key Features'),
            _buildFeatureList([
              'Realistic, weighty gunplay',
            ...
            _buildGameModes([
              {'name': 'Campaign', 'desc': 'Story-driven single-player'},
              {'name': 'Multiplayer', 'desc': 'Class-based online battles'},
              {'name': 'Skirmish', 'desc': 'Offline bot matches'},
            ]),
