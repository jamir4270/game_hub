...
class DiabloPage extends StatefulWidget {
  const DiabloPage({super.key});

  @override
  State<DiabloPage> createState() => _DiabloPageState();
}

class _DiabloPageState extends State<DiabloPage> {
  double _rating = 0;
  ...
  Future<void> _loadPrefs() async {
    ...
    setState(() {
      _rating = prefs.getDouble('diablo_rating') ?? 0;
      _favorite = prefs.getBool('diablo_favorite') ?? false;
      _review = prefs.getString('diablo_review') ?? '';
  ...
  Future<void> _loadReviews() async {
    final prefs = await SharedPreferences.getInstance();
    setState(() {
      _reviews = prefs.getStringList('diablo_reviews') ?? [];
    });
  }

  Future<void> _savePrefs() async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setDouble('diablo_rating', _rating);
    await prefs.setBool('diablo_favorite', _favorite);
    await prefs.setString('diablo_review', _review);
  }

  Future<void> _addReview(String review) async {
    if (review.trim().isEmpty) return;
    final prefs = await SharedPreferences.getInstance();
    final reviews = prefs.getStringList('diablo_reviews') ?? [];
    reviews.add(review.trim());
    await prefs.setStringList('diablo_reviews', reviews);
    setState(() {
      _reviews = reviews;
      _review = '';
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text('Diablo', style: TextStyle(color: textColor)),
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            Text('Diablo', style: Theme.of(context).textTheme.headlineMedium?.copyWith(fontWeight: FontWeight.bold, color: textColor)),
            const SizedBox(height: 8),
            Wrap(
              spacing: 16,
              runSpacing: 8,
              children: [
                Chip(label: Text('Action RPG', style: TextStyle(color: textColor)), backgroundColor: cardColor),
                Chip(label: Text('Dungeon Crawler', style: TextStyle(color: textColor)), backgroundColor: cardColor),
                Chip(label: Text('M (Mature)', style: TextStyle(color: textColor)), backgroundColor: cardColor),
                Chip(label: Text('PC, PlayStation, Mac', style: TextStyle(color: textColor)), backgroundColor: cardColor),
              ],
            ),
...
''', style: TextStyle(color: textColor)),
            ...
            Text(
              'Set in the dark, medieval world of Sanctuary, Diablo pits players against the Lord of Terror. Explore randomized dungeons, slay horrific creatures, and uncover ancient artifacts in this genre-defining action RPG.',
            ...
            _buildFeatureList([
              ...
              'Online multiplayer via Battle.net',
              'Iconic boss battles',
              'Loot-based progression',
            ]),
            _buildSectionTitle('Game Modes'),
            _buildGameModes([
              {'name': 'Single Player', 'desc': 'Story-driven dungeon crawling'},
              {'name': 'Multiplayer', 'desc': 'Cooperative or competitive over LAN or Battle.net'},
            ]),
